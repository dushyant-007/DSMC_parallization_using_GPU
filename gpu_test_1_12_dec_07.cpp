...
void SET_INITIAL_STATE_1D()
{
    ...
    ILEVEL=0;
    //
    i_allocate(ILEVEL+2,MDIV+1,JDIV);
    ...
    d_allocate(14,NCELLS+1,MSP+1,VARSP);
    d_allocate(37+MSP,3,VARS);
    d_allocate(11+MSP,NCELLS+1,MSP+1,CS);
    d_allocate(10,3,MSP+1,3,CSS);
    d_allocate(7,3,CSSS);
    ...
    if(MMVM >= 0){
        ...
        d_allocate(MSP+1,MMVM+1,152,VIBFRAC);
    ...
    for(N=1;N<=NCELLS;N++){
        ...
        for(M=1;M<=NCIS;M++){
            L=ICELL[N]+M;
            XMIN=CELL[2][N]+double(M-1)*DDIV/double(NCIS);
    ...
    file_9<< "Collision time in the stream is "<<CTM;
    //
    for(L=1;L<=MSP;L++){
        FP[L]=0.e00;
        for(M=1;M<=MSP;M++){
        ...
        for(L=1;L<=MSP;L++){
            ...
            for(K=1;K<=ISECS+1;K++){
                ...
                if((K == 1) || (ISECS == 1)){
                    ...
                    while(N < M){
                        if((JFX == 1) || (K == 1))
                            A=pow((pow(XMIN,JFX)+((double(N)-0.5e00)/double(M))*pow((XMAX-XMIN),JFX)),(1.e00/double(JFX)));
                        else{
...
void OUTPUT_RESULTS()
{
    ...
    // string to char array 
            ...
            //dout
            file_3.open(char_array, ios::out);
            if(file_3.is_open()){
            ...
            if(MSP==1){
                ...
                for(N=1;N<=MSP;N++){
                    ...
                    for(M=1;M<=MSP;M++)
                    {
                        ...
                        file_3<<"with species "<<M<<" "<<TCOL[N][M]/((double)(NM)*FSP[N][1]*(FTIME-TISAMP))/THCOL[N][M]<<endl;
                    }
                }
                file_3<<endl;
                for(N=1;N<=MSP;N++){
                ...
                if(NSEL>0)
                {
                    ...
                    for(KL=1;KL<=NSEL;KL++)
                    {
                        ...
                        else
                        {
                            //dout
                            //                            RANDOM_NUMBER(RANF);
                            RANF=(double)(rand()%100001)/100000;//((double)rand()/(double)RAND_MAX);
