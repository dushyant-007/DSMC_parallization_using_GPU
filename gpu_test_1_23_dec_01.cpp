...
{
        x-- ;
        cudaMallocManaged(&b , (x+1)*sizeof(int) ) ;
...
void FIND_CELL_1D(double &X,int &NCC,int &NSC)
{
    ...
    //
    if(get(JDIV ,0,ND) < 0){    //the division is a level 0 (no sub-division) sampling cell
        NSC=-get(JDIV ,0,ND);
        //  IF (IFX == 0)
        ...
        for(N=1;N<=ILEVEL;N++){
            ...
            for(L=1;L<=2;L++){  //over the two level 1 subdivisions
                if(((L == 1) && (FRAC < DSC)) || ((L == 2) || (FRAC >= DSC))){
                    M=get(JDIV ,N-1,M)+L;  //the address in JDIV
                    if(get(JDIV ,N,M) < 0){
                        NSC=-get(JDIV ,N,M);
...
void REFLECT_1D(int &N,int J,double &X)
{
    ...
    get (CSS , 0,J,L,1)=get (CSS , 0,J,L,1)+1.e00;
    get (CSS , 1,J,L,1)=get (CSS , 1,J,L,1)+WF;
    get (CSS , 2,J,L,1)=get (CSS , 2,J,L,1)+WF*get(PV  ,1,N)*get(SP ,5,L);
    get (CSS , 3,J,L,1)=get (CSS , 3,J,L,1)+WF*(get(PV  ,2,N)-VSURF[J])*get(SP ,5,L);
    get (CSS , 4,J,L,1)=get (CSS , 4,J,L,1)+WF*get(PV  ,3,N)*get(SP ,5,L);
    A=pow(get(PV  ,1,N),2)+pow((get(PV  ,2,N)-VSURF[J]),2)+pow(get(PV  ,3,N),2);
    get (CSS , 5,J,L,1)=get (CSS , 5,J,L,1)+WF*0.5e00*get(SP ,5,L)*A;
    if(get(ISPR ,1,L) > 0) get (CSS , 6,J,L,1)=get (CSS , 6,J,L,1)+WF*PROT[N];
    if(MELE > 1) get (CSS , 8,J,L,1)=get (CSS , 8,J,L,1)+WF*PELE[N];
    if(MMVM > 0){
        if(get(ISPV  ,L) > 0){
            for(K=1;K<=get(ISPV  ,L);K++)
                get (CSS , 7,J,L,1)=get (CSS , 7,J,L,1)+WF*double(get (IPVIB , K,N))*BOLTZ*get(SPVM ,1,K,L);
        }
    }
    ...
    //
    get (CSS , 2,J,L,2)=get (CSS , 2,J,L,2)-WF*get(PV  ,1,N)*get(SP ,5,L);
    get (CSS , 3,J,L,2)=get (CSS , 3,J,L,2)-WF*(get(PV  ,2,N)-VSURF[J])*get(SP ,5,L);
    get (CSS , 4,J,L,2)=get (CSS , 4,J,L,2)-WF*get(PV  ,3,N)*get(SP ,5,L);
    A=pow(get(PV  ,1,N),2)+pow((get(PV  ,2,N)-VSURF[J]),2)+pow(get(PV  ,3,N),2);
    get (CSS , 5,J,L,2)=get (CSS , 5,J,L,2)-WF*0.5e00*get(SP ,5,L)*A;
    if(get(ISPR ,1,L) > 0) get (CSS , 6,J,L,2)=get (CSS , 6,J,L,2)-WF*PROT[N];
    if(MELE > 1) get (CSS , 8,J,L,2)=get (CSS , 8,J,L,2)-WF*PELE[N];
    if(MMVM > 0){
        if(get(ISPV  ,L) > 0){
            for(K=1;K<=get(ISPV  ,L);K++)
                get (CSS , 7,J,L,2)=get (CSS , 7,J,L,2)-WF*double(get (IPVIB , K,N))*BOLTZ*get(SPVM ,1,K,L);
        }
    }
    A=pow(get(PV  ,1,N),2)+pow(get(PV  ,2,N),2)+pow(get(PV  ,3,N),2);
    B=fabs(get(PV  ,1,N));
            ...
            if(get(ISPRC ,L,M) > 0){
                ...
                for(I=1;I<=2;I++){
                    ...
                    for(J=0;J<=JMAX;J++){
                        ...
                        CQAX(A,X,B);
                        get (VRRD , I,L,M,J)=B*exp(-double(J)*get(SPVM ,1,K,N)/T);
                        CUR=CUR+get (VRRD , I,L,M,J);
                    }
                    B=0.e00;
                    for(J=0;J<=JMAX;J++){
                        B=B+get (VRRD , I,L,M,J)/CUR;
                        get (SPRP , I,L,M,J)=B;
                        //WRITE (9,*) 'CDF level dissoc',J,SPRP(I,L,M,J)
                        file_9<< "CDF level dissoc "<<J<<" "<<get (SPRP , I,L,M,J);
                    }
                }
            }
        }
                    ...
                    if(get(SPEX  ,3,K,L,M) > 0.e00){         //exothermic (reverse) exchange reaction
                        ...
                        for(I=1;I<=2;I++){
                            ...
                            for(J=0;J<=JMAX;J++){
                                TVD=get(SPVM ,1,get(ISPEX  ,K,5,L,M),MOLSP)/T;
                                ZVT=1.e00/(1.e00-exp(-TVD));
                                get (BFRAC , J,I)=exp(-J*get(SPVM ,1,get(ISPEX  ,K,5,L,M),MOLSP)/T)/ZVT;    //Boltzmann fraction
                                get (VRREX , I,K,L,M,J)=get (VRREX , I,K,L,M,J)*get (BFRAC , J,I);
                                //              WRITE (*,*) 'Contribution',I,J,VRREX(I,K,L,M,J)
                                for(MM=0;MM<=J;MM++)
                                    get (TOT , J,I)=get (TOT , J,I)+get (VRREX , I,K,L,M,MM);
                            }
                        }
                        //
                        for(I=1;I<=2;I++){
                            for(J=0;J<=JMAX;J++){
                                get (SPREX , I,K,L,M,J)=get (TOT , J,I);
                                if(J == JMAX) get (SPREX , I,K,L,M,J)=1.e00;
                                //WRITE (9,*) 'Cumulative',I,J,SPREX(I,K,L,M,J)
                                file_9<<"Cumulative "<<I<<" "<<J<<" "<<get (SPREX , I,K,L,M,J);
                            }
                        }
                    }
                }
...
void SAMPLE_FLOW()
{
    ...
    for(N=1;N<=NM;N++){
        ...
        if((NC > 0) && (NC <= NCELLS)){
            ...
            get (CS , 0,NC,LS)=get (CS , 0,NC,LS)+1.e00;
            get (CS , 1,NC,LS)=get (CS , 1,NC,LS)+WF;
            if(N == 1)
            
            for(M=1;M<=3;M++){
                get (CS , M+1,NC,LS) = get (CS , M+1,NC,LS) + WF*get(PV  ,M,N) ;
                get (CS , M+4,NC,LS)=get (CS , M+4,NC,LS)+ WF*pow(get(PV  ,M,N),2) ;
            }
            if(MMRM > 0) get (CS , 8,NC,LS)=get (CS , 8,NC,LS)+WF*PROT[N];
            if(MELE > 1) get (CS , 9,NC,LS)=get (CS , 9,NC,LS)+WF*PELE[N];
            if(MMVM > 0){
                if(get(ISPV  ,LS) > 0){
                    for(K=1;K<=get(ISPV  ,LS);K++)
                        get (CS , K+9,NC,LS)=get (CS , K+9,NC,LS)+WF*double(get (IPVIB , K,N));
                }
            }
        }
        else{
    ...
    for(int II = 0 ; II <11 ; II++)
        {
            cout << "get (CS , "<<II<<",1,1] =" <<get (CS , II , 1, 1 ) <<endl ;
        }
    //
...
void OUTPUT_RESULTS()
{
    ...
    d_allocate(MMVM+1 , MSP+1 , TV ) ;
    
    d_allocate(MSP+1 , TVIB) ;
    
    
    d_allocate(NCELLS+1 , MMVM+1 , MSP +1 , DF) ;
    
    //VDOF= new double[MSP+1];
    d_allocate(MSP+1 , VDOF );
    //TEL = new double[MSP+1];
    d_allocate(MSP+1 , TEL) ;
    //ELDOF = new double[MSP+1];
    d_allocate(MSP+1 , ELDOF) ;
    //PPA = new double[MSP+1];
    d_allocate(MSP+1 , PPA) ;
    //NMS = new int[MSP+1];
    i_allocate(MSP+1 , NMS) ;
    
    
    d_allocate(MSP+1 , MSP+1 , THCOL) ;
    
    d_allocate(MSP+1 , SDOF) ;
    
    d_allocate(NCELLS+1 , CDTM) ;
        ...
        if(ITYPE[JJ]==2)
        {
            ...
            for( L=1;L<=MSP;L++)
            {
                for(J=0;J<=8;J++)
                {
                    for(IJ=1;IJ<=2;IJ++)
                    {
                        get (SUMS , J,IJ)=get (SUMS , J,IJ)+get (CSS , J,JJ,L,IJ);
                    }
                }
            }
            get (VARS , 0,JJ)=get (SUMS , 0,1);
            get (VARS , 1,JJ)=get (SUMS , 1,1);
            get (VARS , 2,JJ)=get (SUMS , 1,2);
            get (VARS , 3,JJ)=get (SUMS , 1,1)*A;
            get (VARS , 4,JJ)=get (SUMS , 1,2)*A;
            get (VARS , 5,JJ)=get (SUMS , 2,1)*A;
            get (VARS , 6,JJ)=get (SUMS , 2,2)*A;
            get (VARS , 7,JJ)=get (SUMS , 3,1)*A;
            get (VARS , 8,JJ)=get (SUMS , 3,2)*A;
            get (VARS , 9,JJ)=get (SUMS , 4,1)*A;
            get (VARS , 10,JJ)=get (SUMS , 4,2)*A;
            get (VARS , 11,JJ)=get (SUMS , 5,1)*A;
            get (VARS , 12,JJ)=get (SUMS , 5,2)*A;
            get (VARS , 13,JJ)=get (SUMS , 6,1)*A;
            get (VARS , 14,JJ)=get (SUMS , 6,2)*A;
            get (VARS , 15,JJ)=get (SUMS , 7,1)*A;
            get (VARS , 16,JJ)=get (SUMS , 7,2)*A;
            get (VARS , 33,JJ)=get (SUMS , 8,1)*A;
            get (VARS , 34,JJ)=get (SUMS , 8,2)*A;
            //   VARS(17,JJ)=SUMS(9,1)*A        //--SURFACE REACTIONS NOT YET IMPLEMENTED
            //   VARS(18,JJ)=SUMS(9,2)*A
            if(get (CSSS , 1,JJ)>1.e-6)
            {
                get (VARS , 19,JJ)=get (CSSS , 3,JJ)/get (CSSS , 2,JJ); ////--n.b. must be modified to include second component in 3D
                get (VARS , 20,JJ)=(get (CSSS , 4,JJ)-get (CSSS , 2,JJ)*get (VARS , 19,JJ)*get (VARS , 19,JJ))/(get (CSSS , 1,JJ)*3.e00*BOLTZ)-TSURF[JJ];
                get (VARS , 19,JJ)=get (VARS , 19,JJ)-VSURF[JJ];
                if(get (CSSS , 6,JJ)>0e00)
                {
                    get (VARS , 21,JJ)=(2.e000/BOLTZ)*(get (CSSS , 5,JJ)/get (CSSS , 6,JJ))-TSURF[JJ];
                }
                else
                {
                    get (VARS , 21,JJ)=0.e00;
                }
            }
            else
            {
                get (VARS , 19,JJ)=0.e00;
                get (VARS , 20,JJ)=0.e00;
                get (VARS , 21,JJ)=0.e00;
            }
            get (VARS , 22,JJ)=(get (SUMS , 2,1)+get (SUMS , 2,2))*A;
            get (VARS , 23,JJ)=(get (SUMS , 3,1)+get (SUMS , 3,2))*A;
            get (VARS , 24,JJ)=(get (SUMS , 4,1)+get (SUMS , 4,2))*A;
            get (VARS , 25,JJ)=(get (SUMS , 5,1)+get (SUMS , 5,2))*A;
            get (VARS , 26,JJ)=(get (SUMS , 6,1)+get (SUMS , 6,2))*A;
            get (VARS , 27,JJ)=(get (SUMS , 7,1)+get (SUMS , 7,2))*A;
            get (VARS , 28,JJ)=(get (SUMS , 9,1)+get (SUMS , 9,2))*A;
            get (VARS , 29,JJ)=get (VARS , 11,JJ)+get (VARS , 13,JJ)+get (VARS , 15,JJ)+get (VARS , 33,JJ);
            get (VARS , 30,JJ)=get (VARS , 12,JJ)+get (VARS , 14,JJ)+get (VARS , 16,JJ)+get (VARS , 34,JJ);
            get (VARS , 31,JJ)=get (VARS , 29,JJ)+get (VARS , 30,JJ);
            get (VARS , 35,JJ)=get (VARS , 33,JJ)+get (VARS , 34,JJ);
            for(L=1;MSP;L++)
            {
                if(get (SUMS , 1,1)>0)
                {
                    get (VARS , 35+L,JJ)=100*get (CSS , 1,JJ,L,1)/get (SUMS , 1,1);
                }
                else
                {
                    get (VARS , 35+L,JJ)=0.0;
                }
            }
        }
    }
    ...
    for(N=1;N<=NCELLS;N++)
    {
        ...
        for(L=1;L<=MSP;L++)
        {
           // dsuedit
            ...
            SUM[0]=SUM[0]+get (CS , 0,N,L);
            SUM[1]=SUM[1]+get (CS , 1,N,L);
            SUM[2]=SUM[2]+get (SP , 5,L)*get (CS , 1,N,L);

            
            for(K=1;K<=3;K++)
            {
                SUM[K+2] = SUM[K+2]+get (SP , 5,L)*get (CS , K+1,N,L);
                if(get (CS , 1,N,L)>0.1e00)
                {
                    get (VARSP , K+1,N,L)=get (CS , K+4,N,L)/get (CS , 1,N,L);
                    //--VARSP(2,3,4 are temporarily the mean of the squares of the velocities
                    get (VARSP , K+8,N,L)=get (CS , K+1,N,L)/get (CS , 1,N,L);
                }
            }
            SUM[6]=SUM[6]+get (SP , 5,L)*(get (CS , 5,N,L)+get (CS , 6,N,L)+get (CS , 7,N,L));
            SUM[10]=SUM[10]+get (SP , 5,L)*get (CS , 5,N,L);
            SUM[12]=SUM[11]+get (SP , 5,L)*get (CS , 6,N,L);
            SUM[12]=SUM[12]+get (SP , 5,L)*get (CS , 7,N,L);
            SUM[13]=SUM[13]+get (CS , 9,N,L);
            if(get (CS , 1,N,L)>0.5e00)
                SUM[7]=SUM[7]+get (CS , 5,N,L)+get (CS , 6,N,L)+get (CS , 7,N,L);
            if(get(ISPR ,1,L)>0)
            {
                SUM[8]=SUM[8]+get (CS , 8,N,L);
                SUM[9]=SUM[9]+get (CS , 1,N,L)*get(ISPR ,1,L);
            }
        }
        AVW=0;
        for(L=1;L<=MSP;L++)
        {
            get (VARSP , 0,N,L)=get (CS , 1,N,L) ;
            get (VARSP , 1,N,L)=0.e00;
            get (VARSP , 6,N,L)=0.0;
            get (VARSP , 7,N,L)=0.0;
            get (VARSP , 8,N,L)=0.0;
            if(SUM[1]>0.1)
            {
                get (VARSP , 1,N,L)=get (CS , 1,N,L)/SUM[1];
                AVW=AVW+get (SP , 3,L)*get (CS , 1,N,L)/SUM[1];
                if(get(ISPR ,1,L)>0 && get (CS , 1,N,L)>0.5)
                    get (VARSP , 6,N,L)=(2.e00/BOLTZ)*get (CS , 8,N,L)/((double)(get(ISPR ,1,L))*get (CS , 1,N,L));
            }
            get (VARSP , 5,N,L)=0;
            for(K=1;K<=3;K++)
            {
                get (VARSP , K+1,N,L)=(get (SP , 5,L)/BOLTZ)*(get (VARSP , K+1,N,L)-pow(get (VARSP , K+8,N,L),2));
                get (VARSP , 5,N,L)=get (VARSP , 5,N,L)+get (VARSP , K+1,N,L);
            }
            get (VARSP , 5,N,L)=get (VARSP , 5,N,L)/3.e00;
            get (VARSP , 8,N,L)=(3.e00*get (VARSP , 5,N,L)+(double)get(ISPR ,1,L)*get (VARSP , 6,N,L))/(3.e00+(double)(get(ISPR ,1,L)));
        }
        if(IVB==0)
        ...
        if(SUM[1]>0.5)
        {
            ...
            if(MMVM>0)
            {
                for(L=1;L<=MSP;L++)
                {
                    ...
                    if(get(ISPV  ,L) > 0)
                    {
                        for(K=1;K<=get(ISPV  ,L);K++)
                        {
                            ...
                            {
                                get(TV , K,L)=get(SPVM ,1,K,L)/log(1.0+get (CS , 1,N,L)/get (CS , K+9,N,L)) ;//--Eqn.(4.45)
                                get(DF , N,K,L)=2.0*(get (CS , K+9,N,L)/get (CS , 1,N,L))*log(1.0+get (CS , 1,N,L)/get (CS , K+9,N,L)); //--Eqn. (4.46)
                            }
                ...
                for(L=1;L<=MSP;L++)
                {
                    ...
                    if(get(ISPV  ,L) > 0)
                    {
                        VDOFM=VDOFM+VDOF[L]*get (CS , 1,N,L)/A;
                        TVIBM=TVIBM+TVIB[L]*get (CS , 1,N,L)/A;
                    }
                }
                get (VAR , 10,N)=TVIBM;
            }
            for(L=1;L<=MSP;L++)
            {
                if(get (VARSP , 0,N,L)>0.5)
                {
                    //--convert the species velocity components to diffusion velocities
                    for(K=1;K<=3;K++)
                    {
                        get (VARSP , K+8,N,L)=get (VARSP , K+8,N,L)-get (VAR , K+4,N);
                    }
                    if(MELE>1)
                    {
                        ...
                        if(MELE>1)
                        {
                            ...
                            {
                                if(get (VARSP , 5,N,L)>1.e00)
                                {
                                    C=get(QELC ,2,M,L)/get (VARSP , 5,N,L);
                            ...
                            if(B>1.e-10)
                            {
                                TEL[L]=(get (CS , 9,N,L)/get (CS , 1,N,L))/(BOLTZ*B/A);
                            }
                            else
                                TEL[L]=get (VAR , 11,N);
                            get (VARSP , 12,N,L)=TEL[L];
                            ELDOF[L]=0.e00;
                            if(get (VARSP , 5,N,L)>1.e00)
                                ELDOF[L]=2.e00*(get (CS , 9,N,L)/get (CS , 1,N,L))/(BOLTZ*get (VARSP , 5,N,L) ) ;
                            if(ELDOF[L]<0.01)
                            {
                                get (VARSP , 12,N,L)=get (VAR , 11,N);
                            }
                        }
                        else
                        {
            ...
            if(MELE>1)
            {
                ...
                if(C>0.e00)
                {
                    ...
                    for(L=1;L<=MSP;L++)
                    {
                        if(ELDOF[L]>1.e-5)
                        {
                            A=A+get (VARSP , 12,N,L)*get (CS , 1,N,L);
                            B=B+get (CS , 1,N,L);
                        }
                    }
                    get (VAR , 22,N)=A/B;
            ...
            if(MMVM>0)
            {
                ...
                for(L=1;L<=MSP;L++)
                {
                    if(MELE>1){
                        ...
                        get (VARSP , 8,N,L)=(3.0*get (VARSP , 5,N,L)+get(ISPR ,1,L)*get (VARSP , 6,N,L)+VDOF[L]*get (VARSP , 7,N,L)+ELDOF[L]*get (VARSP , 12,N,L))/SDOF[L];
                    }
                    else{
                        SDOF[L]=3.e00+get(ISPR ,1,L)+VDOF[L];
                        get (VARSP , 8,N,L)=(3.0*get (VARSP , 5,N,L)+get(ISPR ,1,L)*get (VARSP , 6,N,L)+VDOF[L]*get (VARSP , 7,N,L))/SDOF[L];
                    }
                }
                ...
                for(L=1;L<=MSP;L++)
                {
                    A=A+SDOF[L]*get (VARSP , 8,N,L)*get (CS , 1,N,L);
                    B=B+SDOF[L]*get (CS , 1,N,L);
                }
                get (VAR , 11,N)=A/B ;
            }
    ...
    if(FTIME>0.5e00*DTM)
    {
        ...
        if(ICLASS==0 && MMVM==0 && ISF==0){
            ...
            if(MSP==1){
                ...
                for(N=1;N<=MSP;N++){
                    ...
                    for(M=1;M<=MSP;M++)
                    {
                        THCOL[N,M]=2.e00*(1.e00/SPI)*get (VAR , 3,1)*get (VARSP , 1,1,M)*get (SPM , 2,N,M)*sqrt(2.e00*BOLTZ*get (SPM , 5,N,M)/get (SPM , 1,N,M))*pow(get (VAR , 11,1)/get (SPM , 5,N,M),1.e00-get (SPM , 3,N,M));
        ...
        for(JJ=1;JJ<=2;JJ++)
        {
            if(ITYPE[JJ]==2){
                ...
                file_3<<"Incident sample "<<get (VARS , 0,JJ)<<endl;
                file_3<<"Number flux "<<get (VARS , 3,JJ)<<" /sq m/s"<<endl;
                file_3<<"Inc pressure "<<get (VARS , 5,JJ)<<" Refl pressure "<<get (VARS , 6,JJ)<<endl;
                file_3<<"Pressure "<< get (VARS , 5,JJ)+get (VARS , 6,JJ)<<" N/sq m"<<endl;
                file_3<<"Inc y shear "<<get (VARS , 7,JJ)<<" Refl y shear "<<get (VARS , 8,JJ)<<endl;
                file_3<<"Net y shear "<<get (VARS , 7,JJ)-get (VARS , 8,JJ)<<" N/sq m"<<endl;
                file_3<<"Net z shear "<<get (VARS , 9,JJ)-get (VARS , 10,JJ)<<" N/sq m"<<endl;
                file_3<<"Incident translational heat flux "<<get (VARS , 11,JJ)<<" W/sq m"<<endl;
                if(MMRM>0)
                    file_3<<"Incident rotational heat flux "<<get (VARS , 13,JJ)<<" W/sq m"<<endl;
                if(MMVM>0)
                    file_3<<"Incident vibrational heat flux "<<get (VARS , 15,JJ)<<" W/sq m"<<endl;
                if(MELE>1)
                    file_3<<"Incident electronic heat flux "<<get (VARS , 33,JJ)<<" W/sq m"<<endl;
                file_3<<"Total incident heat flux "<<get (VARS , 29,JJ)<<" W/sq m"<<endl;
                file_3<<"Reflected translational heat flux "<<get (VARS , 12,JJ)<<" W/sq m"<<endl;
                if(MMRM>0)
                    file_3<<"Reflected rotational heat flux "<<get (VARS , 14,JJ)<<" W/sq m"<<endl;
                if(MMVM>0)
                    file_3<<"Reflected vibrational heat flux "<<get (VARS , 16,JJ)<<" W/sq m"<<endl;
                if(MELE>1)
                    file_3<<"Reflected electronic heat flux "<<get (VARS , 34,JJ)<<" W/sq m"<<endl;
                file_3<<"Total reflected heat flux "<<get (VARS , 30,JJ)<<" W/sq m"<<endl;
                file_3<<"Net heat flux "<<get (VARS , 31,JJ)<<" W/sq m"<<endl;
                file_3<<"Slip velocity (y direction) "<<get (VARS , 19,JJ)<<" m/s"<<endl;
                file_3<<"Translational temperature slip"<<get (VARS , 20,JJ)<<" K"<<endl;
                if(MMRM>0)
                    file_3<<"Rotational temperature slip "<<get (VARS , 21,JJ)<<" K"<<endl;
                if(MSP>1)
                {
                    for(L=1;L<=MSP;L++)
                    {
                        file_3<<"Species "<<L<<" percentage "<<get (VARS , L+35,JJ)<<endl;
                    }
                }
            }
        }
    ...
    //WRITE (19,*) FTIME,-get (VARS , 5,1],-get (VARS , 5,1]-get (VARS , 6,1]
    ...
    for(N=1;N<=NCELLS;N++)
    {
        ...
        for(L=1;L<=MSP;L++)
        {
            ...
            A=(get (CS , 1,N,L)/get (VARSP , 1,N,L))*get (SP , 5,L);
            if(get (CS , 1,N,L)>0.5e00){
                C2=C2+0.5e00*(get (CS , 5,N,L)+get (CS , 6,N,L)+get (CS , 7,N,L))*get (SP , 5,L)/A;
                if(get(ISPR ,1,L)>0)
                    C3=C3+get (CS , 8,N,L)/A;
                if(get(ISPV  ,L)>0)
                    C4=C4+get (CS , 10,N,L)*BOLTZ*get(SPVM ,1,1,L)/A;
                if(get(NELL  ,L)>1)
                    C5=C5+get (CS , 9,N,L)/A;
                C6=C6+get (SP , 6,L)*get (CS , 1,N,L)/A;
            }
        }
        C2=C2-C1;
                                        ...
                                        if(get(ISPRC ,LS,MS)>0 && get (ICCELL , 2,N)>2)
                                        {
                                            ...
                                            if(RANF<B)
                                            {
                                                ...
                                                while(IK<0)
                                                {
                                                    ...
                                                    NK=NK+1;
                                                    BB=(get (VAR , 8,NN)-get( SPRT ,1,LSI,MSI))*(get (SPRP , 2,LSI,MSI,NK)-get (SPRP , 1,LSI,MSI,NK))/(get( SPRT ,2,LSI,MSI)-get( SPRT ,1,LSI,MSI))-get (SPRP , 1,LSI,MSI,NK);
                                                    if(RANF<BB)
                                                        IK=NK;
                                                }
                                                get (IPVIB , 1,L)=IK;
                                            ...
                                            if(IEX>0)
                                            {
                                                ...
                                                if(get(SPEX  ,3,IEX,LSI,MSI)>0.e00)
                                                {
                                                    ...
                                                    while(IK<0)
                                                    {
                                                        ...
                                                        // }
                                                        NK=NK+1;
                                                        BB=(get (VAR , 8,NN)-get(SPEX  ,4,IEX,LSI,MSI))*  (get (SPREX , 2,IEX,LSI,MSI,NK)-get (SPREX , 1,IEX,LSI,MSI,NK))/(get(SPEX  ,5,IEX,LSI,MSI)-get(SPEX  ,4,IEX,LSI,MSI))+get (SPREX , 1,IEX,LSI,MSI,NK);
                                                        if(RANF<BB)
                                                            IK=NK;
                                                    }
                                                    if(get(NSLEV ,1 , LS)>0)
