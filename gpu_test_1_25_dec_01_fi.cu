...
{
    if(!x || !y || !z || !w)    cout << "error_4_I" << endl ;
        int r1 = b[b[0]+2] ;
        int r2 = b[b[0]+3] ;
        int r3 = b[b[0]+4] ;
...
{
        if(!x || !y || !z || !w || !u)    cout << "error_5_I" << endl ;
        int r1 = b[b[0]+2] ;
        int r2 = b[b[0]+3] ;
        int r3 = b[b[0]+4] ;
        int r4 = b[b[0]+5] ;
...
{
        if(!x || !y || !z || !w )    cout << "error_4_d" << endl ;
        int r1 = b[(int)b[0]+2] ;
        int r2 = b[(int)b[0]+3] ;
        int r3 = b[(int)b[0]+4] ;
        return b[(((x-1)*r1 + y-1)*r2+z-1)*r3+w] ;
...
{
        if(!x || !y || !z || !w || !u)    cout << "error_5_d" << endl ;
        int r1 = b[(int)b[0]+2] ;
        int r2 = b[(int)b[0]+3] ;
        int r3 = b[(int)b[0]+4] ;
        int r4 = b[(int)b[0]+5] ;
...
void INITIALISE_SAMPLES()
{
    ...
    for(int j=1;j<MSP+1;j++){
        for(int k=1;k<MMVM+1;k++){
            for(int l=0;l<151;l++)
                get (VIBFRAC , j,k,l+1)=0.0;
        }
    }
    for(int k=1;k<MSP+1;k++){
        for(int l=1;l<MMVM+1;l++)
            get (SUMVIB , k,l)=0.0;
    }

    cout << "INITIALISE SAMPLES IS FINIDHED\n" ;
